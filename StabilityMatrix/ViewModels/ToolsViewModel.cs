...
public partial class ToolsViewModel : ObservableObject
{
    ...
    public Visibility ProgressRingVisibility =>
        IsProgressRingActive ? Visibility.Visible : Visibility.Collapsed;

    public Visibility ImagePreviewVisibility =>
        IsProgressRingActive ? Visibility.Collapsed : Visibility.Visible;
    ...
    public List<string> Samplers { get; } = new() { "Euler a", "Euler", "DPM++ 2M Karras" };

    public TextToImageViewModel(
        IA3WebApiManager a3WebApiManager,
        ILogger<TextToImageViewModel> logger,
        ISnackbarService snackbarService,
        PageContentDialogService pageContentDialogService,
    ...
    private async Task SetActiveModelFromApi()
    {
        ...
        if (responseResult is { IsSuccessful: true, Result: not null })
        {
            ...
            var checkpointFile = DiffusionCheckpointFolder
                ?.CheckpointFiles
                .FirstOrDefault(f => f.FileName == options.SdModelCheckpoint);
            logger.LogInformation(
                "Set active checkpoint from api {CheckpointFile}",
    ...
    private async Task OnProgressTrackingTick()
    {
        ...
        logger.LogInformation(
            "Image Progress: {ResponseProgress}, ETA: {ResponseEtaRelative} s",
            response.Progress,
            response.EtaRelative
        );

...
using StabilityMatrix.Avalonia.Workflows;
...
[View(typeof(ComfyOrchastrationPage))]
[Singleton]
public partial class ComfyOrchastrationViewModel() : TabViewModelBase
{
    public override string Header => "Comfy Orchastration";
    ...
    public bool IsDragBlurEnabled => IsCurrentDragTarget;
    ...
    private async Task Execute()
    {
        ...
        {
            await ComfyOrchestration();
        }
        else if (Directory.Exists(FilePath))
        {
            SortFiles(FilePath, out string[] files);
            foreach (string item in files.Skip(SkipFileCount))
            {
                FilePath = item;
                await ComfyOrchestration();
    ...
    private void Reset()
    {
        ...
        DetectionSensitivity = 0.7;
        _isMatched = false;
        this.ImageTaggerFile = string.Empty;
        this.GenderFile = string.Empty;
        this.SegsFile = string.Empty;
        this.SegsData = string.Empty;
        this.ImageTaggerPrompt = null;
        this.ClassifierPrompt = null;
        this.SegsPrompt = null;
        this.UploadedFileName = string.Empty;
        this._imageTaggerText = string.Empty;
        this._scaleBy = 0;
    ...
    async Task ComfyOrchestration()
    {
        IsUploaded = false;
        Logger.Info($"Start Comfy Orchestration");
        ImageTaggerFile = await ImageTagger.TagImages(this);
        GenderFile = await Classifier.SendImageToClassifier(this);
        SegsFile = await ImageTagger.TagSegs(this);
        ProcessSegsFile().Wait();
        ...
        await Detailer.SendImageToDetailer(this);
        PrintLog();
        // clean up
        ImageTagger.CleanUp(this, CleanOptions.All);
    }

    private void SortFiles(string directory, out string[] files)
    {
    ...
    public int ScaleBy
    {
        get
        {
            if (_scaleBy == 0)
            {
                ...
                var imageWidth = GetImageWidth(FilePath);
                if (imageWidth > 0)
                {
                    if (imageWidth > 2000)
                    {
    ...
    public void PrintLog()
    {
        ...
        File.AppendAllText(filePath, "Segs Overlap : " + SegmentsOverlap().ToString() + Environment.NewLine);
        ...
        File.AppendAllText(filePath, "Detection Sensitivity : " + DetectionSensitivity + Environment.NewLine);
        File.AppendAllText(filePath, "Detection Confidence : " + DetectionConfidence + Environment.NewLine);
        File.AppendAllText(
            filePath,
        ...
        File.AppendAllText(filePath, "Detailer Crop Factor : " + DetailerCropFactor + Environment.NewLine);
